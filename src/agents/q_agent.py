...
class QAgent(BaseAgent):
	...
	def _ensure_q_values(self, state_idx: int) -> np.ndarray:
		"""Ensure Q-values are initialized for a given state index."""
	...
	def learn(self, experience: Dict[str, Any]) -> Optional[Dict[str, float]]:
		...
		self._ensure_q_values(state_idx)

		# Get current Q-value
		current_q = self.q_table[state_idx]

		# Calculate target Q-value
		if done:
			target_q = reward
		else:
			self._ensure_q_values(next_state_idx)
